...
#### Logo 库页面（新增）
- **全量图标库** — 871 SVG + 727 PNG Light + 727 PNG Dark = 2325 个文件，311 个唯一图标（来自 @lobehub/icons）
- **格式切换** — 预览弹窗支持 SVG / PNG Light / PNG Dark，左侧变体标签栏
- **分类筛选** — 全部 / 大语言模型 / 图像视频 / 云服务 / Agent工具 / 基础设施
- **搜索与收藏** — 按图标 ID 搜索，收藏功能持久化到 localStorage
- **基于 manifest 加载** — 自动生成 manifest.json 实现秒加载
- **暗色适配** — Mono SVG 图标在暗色模式下自动反色

#### 模型卡片（改进）
- **供应商 Logo** — 40px 圆角容器 + 28px 供应商 Logo 紧邻模型名
- **管理员可设置 Logo** — Logo 选择弹窗，支持搜索，管理员可为每个模型设置自定义 Logo
- **logo_id 字段** — 新增数据库字段支持逐模型 Logo 覆盖
- **垂直居中** — Logo 和模型名正确对齐

### Bug 修复（32+）
- **安全：API Key 前缀认证绕过** — 移除 10 字符 LIKE 匹配
- **安全：注册存储哈希 Key** — 改为存储掩码 Key + Hash
- **安全：SSRF 防护** — Playground fetch 拦截内网地址
- **安全：密码重置失效 session** — token_version 递增
- **安全：Webhook URL 验证** — 拒绝内网 URL
- **计费：流式路径 cost 未置零** — 套餐用户正确显示 0 cost
- **计费：时区符号反转** — 修复 +480 → -480（UTC+8）
- **计费：日趋势时区** — GROUP BY 使用时区偏移
- **计费：Credit 用户检测** — 从 summary.total_credits_used 检测
- **UX：导航进度条到 100%** — 动画到 100% 后再隐藏
- **UX：回到顶部按钮** — 移动端改为 bottom-20
- **UX：反馈提示状态重置** — 页面导航时重置
- **UX：清空仅当前会话** — 不再删除所有会话
- **UX：引用消息发送给 LLM** — 现在包含在消息数组中
- **UX：图片错误处理** — 添加 onerror 处理

### 性能
- **渠道延迟缓存** — 30 秒 TTL 缓存渠道健康数据
- **监控百分位** — 单次窗口函数替代 O(n) OFFSET
- **Prompt 缓存清理** — 确定性 5 分钟定时器替代概率性清理

### i18n
- **500+ 内联三元迁移** — 仪表盘组件使用集中式字典
- **ErrorBoundary i18n** — 支持 labels prop
- **ConfirmDialog i18n** — 默认值从字典获取
- **NotificationBell** — 筛选标签使用字典 key
...
#### MCP 生态广场
- **分页** — 每页 9 个，支持"加载更多"，筛选切换时重置
- **Featured 徽章** — 推荐节点显示琥珀色星标
- **搜索下拉关闭** — 点击外部区域自动关闭搜索下拉
- **Tab URL 持久化** — Tab 状态同步到 URL（`?tab=mcp`），刷新和分享不丢失
- **i18n 修复** — "服务类型"和"分类"标题使用 i18n 键
- **标签搜索** — 大小写不敏感

#### MCP 详情页
- **真实反馈系统** — 复用 CommentSection 组件，支持评分、评论、点赞、回复
- **Featured 指示器** — 标题区琥珀色星标
- **Tips 准确性** — 本地/托管 MCP 显示不同提示文案
- **复制失败 Toast** — 剪贴板错误时显示 Toast
- **i18n 合规** — 20+ 硬编码三元表达式全部替换为 i18n 键

#### 资源注册表
- **真实 MCP 服务器** — 17 个节点全部来自 `modelcontextprotocol/servers` (MIT) 及 established repos
- **类型安全** — `mcpCategory` 使用 `McpCategory` 联合类型
- **空分类** — 移除无数据的 `communication` 和 `finance`
- **客户端技能描述** — `descriptionZh` 使用 Agent 技能市场的中文标题
...
- **部署类型徽章** — MCP 卡片显示 Hosted（紫色）/ Local（蓝色）徽章
- **排序下拉** — 推荐优先 / 名称排序 / 使用量排序，特定 Tab 时可见
...
#### 新增页面
- **功能总览**（`/docs/features`）— 流式输出、函数调用、JSON 模式、视觉多模态、Prompt 缓存、模型路由，每个功能附带代码示例
- **模型与定价**（`/docs/models-pricing`）— 热门模型价格表、计费方式说明、API 获取模型列表示例
- **常见问题**（`/docs/faq`）— 15 个 FAQ 覆盖 5 个类别（账户、计费、模型、技术、集成），可折叠手风琴组件

#### 新增组件
- **ResponseSchema** — 可折叠的请求/响应 JSON Schema 展示，带参数表（名称、类型、必填、描述）
- **FaqAccordion** — 支持深度链接的手风琴组件，展开/收起动画，React Context 控制单开模式
- **FeedbackPrompt** — "这个页面有帮助吗？" 反馈组件，支持点赞/踩、可选评论、localStorage 按页面持久化

#### 内容增强
- **端点页** — 新增可展开的请求参数表、响应 JSON 示例、每个端点组的 curl 示例
- **错误码页** — 每个错误码新增故障排除（常见原因 + 解决方案），通用重试策略（指数退避）
- **SDK 页** — 新增"最快接入"OpenAI SDK 直接替换说明、LangChain/Vercel AI SDK 示例、流式示例、错误处理示例
- **流式页** — 新增 Python/Node.js SDK 流式示例、Anthropic SDK 流式、流式错误处理
- **速率限制页** — 新增限流等级对比表（默认/标准/Pro/企业）、查看当前限制说明、最佳实践扩展

#### 架构调整
- **侧边栏** — 从 6 个零散分组重组为 4 个逻辑分组：快速开始、API 参考、指南、资源
- **落地页** — 新增快速统计徽章、"3 步开始"内联引导、重组卡片分组匹配新侧边栏、"需要帮助？" CTA 区域
- **布局** — FeedbackPrompt 组件添加到每个文档页面
...
#### 新功能（5 项）
- **系统状态页** — `/status` 页面，显示上游供应商健康状态和基础设施检查
- **键盘快捷键** — 按 `?` 打开快捷键参考面板
- **API 密钥轮换** — 一键生成新密钥，停用旧密钥，显示新密钥值
- **API 更新日志** — `/docs/changelog` 版本历史时间线
- **Prompt 模板库** — 本地存储的提示词模板保存/加载

#### 功能扩展（7 项）
- **统计卡片可点击** — 仪表盘卡片跳转到相关页面
- **搜索分类过滤** — 搜索页面分类标签（全部/模型/文档/错误码）
- **导航栏搜索** — 桌面端持久搜索输入框
- **Toast 操作按钮** — 支持撤销操作的 Toast
- **通知偏好** — 设置页按事件类型配置通知
- **设置搜索** — 搜索栏过滤设置卡片
- **引导进度** — Dashboard 引导清单带进度条

#### 视觉与设计（12 项）
- **页面过渡** — 路由切换淡入动画
- **加载进度条** — 渐变顶部进度条
- **Shimmer 骨架屏** — 替换纯脉冲动画
- **计数动画** — `useCountUp` 数字动画钩子
- **提供商指示器** — 模型卡片彩色圆点
- **活动源颜色** — 时间线按供应商颜色编码
- **导航下划线** — 滑动下划线动画
- **渐变按钮** — 按钮渐变变体
- **图表 CSS 变量** — 图表颜色从自定义属性解析
- **响应式标题** — 全站 `text-xl md:text-2xl`
- **移动端底部导航** — 固定标签栏（5 个关键页面）
- **404 页面增强** — 渐变标题和品牌插图

#### 交互优化（8 项）
- **Playground 参数折叠** — 高级参数可折叠
- **Playground 预设** — 均衡/创意/精确/代码预设
- **对话导出** — Playground 导出 Markdown
- **未保存离开确认** — `useUnsavedChanges` 钩子
- **自动保存** — 预算字段 2 秒防抖自动保存
- **乐观更新** — API 切换即时 UI 反馈
- **错误分类** — HTTP 状态特定错误提示
- **流式指标** — TTFB 和 tokens/sec 显示

#### 微交互（5 项）
- **按钮缩放反馈** — `active:scale-[0.97]`
- **Toast 动画** — 成功弹跳/错误抖动
- **空状态动画** — 渐入效果
- **卡片抬升** — 弹簧缓动 hover 效果
- **注册纸屑** — 五彩庆祝粒子
...
#### 后台与会话（2 项）
- **投递日志** — Webhook 投递状态对话框
- **活跃会话** — 个人资料页会话管理

#### 安全与 Bug 修复（7 项）
- **安全** — 中间件 JWT 签名验证
- **安全** — `hashApiKey` 使用服务端密钥
- **安全** — SQL 参数化查询
- **安全** — CORS `*` 回退修复
- **安全** — 密码最小长度统一 8
- **Bug** — 面包屑阈值修复
- **Bug** — 清除筛选按钮修复
- **Bug** — 分析页统计卡片（总调用/总Tokens/花费）现在响应 7/14/30 天选择器，不再始终显示全部总计
- **Bug** — 分析页饼图、消费趋势、模型延迟、错误率、缓存率、延迟趋势统一响应时间范围选择器（7/14/30/按天/按小时）
- **Bug** — 缓存节省查询改用 `DATE('now', 'start of month')` 修复时区格式不匹配问题
- **UI** — 缓存 ROI 卡片更名为"本月缓存 ROI"，明确数据范围为当月
- **UI** — 移除月 Tokens 卡片中的缓存命中率冗余行
- **UI** — 模型网格从 3 列调整为 4 列布局
- **UI** — Glass 卡片视觉效果增强，改进 backdrop-blur 和聚光灯效果
- **UI** — Token Plan 页面全面美化：Hero 区渐变标题 + 浮动球体动画
- **UI** — Shimmer 流光骨架屏替换脉冲加载
- **UI** — 卡片 staggered 级联淡入入场动画
- **UI** — FAQ 手风琴升级为玻璃卡片样式
- **UI** — 每张套餐卡片拥有主题色呼吸发光边框（box-shadow spread 技术，圆角完美贴合）
- **UI** — Popular 卡呼吸频率 3s 更快更强，卡片上浮突出
- **UI** — 每月额度数字使用主题色高亮，底部显示 credits 单位
- **UI** — 订阅按钮使用套餐主题色渐变；当前套餐按钮加粗边框
- **UI** — Hero 信任条展示实时站点数据（总调用数、模型数、活跃用户）
- **UI** — 每张卡片显示百万 Token 均价，联动币种切换
- **UI** — 月/年切换时价格数字 CountUp 翻滚动效
- **UI** — Popular 卡渐变分隔光条（页眉/内容之间）
- **UI** — 所有 Section 滚动渐入动画（IntersectionObserver）
- **UI** — 底部 CTA 玻璃卡片区（查看文档 / 联系销售）
- **Feature** — 新增 `/docs/ai-tools/` 文档区：AI 工具总览 + 8 个工具配置页面
- **Feature** — 套餐页工具卡片（2×4 网格）跳转到对应文档；支持 SVG Logo 显示
- **Integration** — 安装 `@lobehub/icons` AI 品牌图标库
...
- **自定义滚动条** — 消息区域原生 webkit 滚动条样式化，`6px` 宽，日/夜模式自适应颜色
- **滚动条可见性** — 轨道 `5%` 透明度，滑块 `20%` 透明度，hover 加深到 `35%`
- **输入框对齐** — `items-end` → `items-start`，`+` 按钮 `pt-1.5`，发送按钮 `w-10 min-h-[2.5rem]`
- **思考模式独立开关** — 从 `+` 菜单移出为独立 `Brain` Toggle；切换模型自动重置
- **图片即时清除** — `setAttachedImages([])` 移到发送前
- **模型能力配置** — 用户侧 localStorage 存储，每个模型独立配置 Vision/Reasoning/Tools 能力；不满足条件时对应按钮隐藏/禁用
- **MCP 风格工具面板** — 卡片式布局 + Toggle 开关 + Tavily Key 输入；移除 Bing；自定义 MCP 预留位
- **URL 抓取** — 服务器代理 `/api/playground/tools` 绕过 CORS；15000 字符截断保护；加载动画
- **思考过程独立气泡** — 琥珀色独立折叠气泡，推理内容到达即显示，正文气泡分离
- **LaTeX 公式渲染** — 集成 KaTeX 引擎，`$...$` 和 `$$...$$` 在正文和推理中均渲染为数学符号
- **MarkdownRenderer 升级** — 新增 h4/h5/h6 标题、`*` 无序列表、`---`/`***`/`___` 分割线、`*斜体*`、`$公式$`
- **发送按钮旋转动画** — 移除状态栏转圈，停止按钮内嵌 `Loader2`
- **推理文字样式** — `text-[12px] text-muted-foreground/90` 区分正文
- **Token 用量持久化** — 每条 `ChatMessage` 自带 `usage` 字段，跨会话回显
- **图片上传修复** — 解决 `Image` 命名冲突（lucide-react 导入与 DOM `Image()` 构造函数同名）；删除重复的 `img.onload` 覆盖压缩逻辑导致的点击图片无响应问题
- **工具能力锁定** — 在能力编辑弹窗中永久禁用并标注 "即将支持"；暗色模式对比度优化

### 全站审计 #2 — 28 项 UI/UX 与视觉优化
- **Bug 修复** — Playground 能力编辑器 map 缺少 key prop（React 控制台错误）
- **安全修复** — 忘记密码页面不再在 API 响应和 UI 中暴露重置 Token
- **密码显隐** — 修改密码表单的 3 个字段现在使用独立的显示/隐藏切换
- **无障碍** — 头像上传改用 `useRef` + 隐藏 input，键盘 Enter/Space 触发
- **快捷键** — 移除 navbar 的 Ctrl+K 监听，统一由 CommandPalette 处理
- **错误反馈** — 套餐页：`alert()` 替换为 Toast；Promise.all catch 增加 Toast
- **错误反馈** — StatsCards：增加 SWR error 状态和重试提示
- **错误反馈** — API Key 表：所有 CRUD 操作失败时显示 Toast
- **错误反馈** — 倍率页：增加 fetch 错误状态和错误横幅
- **错误反馈** — 控制台页：错误横幅添加 `role="alert"`
- **错误反馈** — 登录页：用户输入时自动清除服务端错误
- **文档侧边栏** — 默认折叠（仅展开当前页面所在 section）；搜索自动展开匹配项
- **文档侧边栏** — 移动端按 Escape 键关闭
- **个人中心** — Tab 状态持久化到 URL（`?tab=settings`），刷新页面不丢失
- **搜索** — 最近搜索删除按钮默认半可见，适配触摸设备
- **视觉** — 浅色主题边框/输入框不透明度从 5% 提升到 8%
- **视觉** — Dialog/Sheet 遮罩从 `bg-black/10` 改为 `bg-black/20`
- **视觉** — 首页段落垂直间距统一为 `py-16 lg:py-20`
- **视觉** — Marquee 动画速度提升（90s→60s, 110s→75s）
- **视觉** — Hero 徽章硬编码颜色改为主题感知 CSS 变量
- **视觉** — ECharts 图表容器固定高度改为响应式 `min-h-[250px] md:min-h-[350px]`
- **视觉** — 默认按钮水平内边距 `px-2.5` → `px-3`
- **视觉** — 价格卡片网格间距 `gap-5` → `gap-6`
- **视觉** — Tabs 最小高度从 32px 提升到 36px，改善触摸目标
- **组件** — LoadingSkeleton 增加 `width`/`shape`/`lines` 选项
- **一致性** — 注册 API 密码最小长度从 6 提升到 8，对齐客户端校验
...
- **全屏聚光灯** — 800px 径向渐变跟随鼠标划过整个页面
- **粒子特效** — 数量减半、速度极慢、星辰风格、自适应日/夜模式
- **微排版工具** — `.label-upper`（10px 大写宽字距）、`.font-mono-force`（API 路径/Key 强制等宽）
- **套餐渐变修复** — 硬编码日/夜模式色值（CSS 变量在重构中丢失）

### 控制台分析重做
- **Sparkline 趋势图** — 统计卡片添加 SVG 面积 sparkline（30 天费用/调用/RPM/TPM 趋势）
- **堆叠面积图** — 柱状图升级为平滑堆叠面积；每条模型线 2px + 15% 透明度填充
- **ECharts 暗色适配** — 透明背景、CSS 变量运行时解析、`useTheme` 驱动主题切换重渲染
- **饼图** — 全宽布局、去除切片边框、前景色标签文字
- **布局** — 左 2/3 主图表 + 右 1/3 粘性活动时间线
- **活动时间线** — 4 行调用明细（模型/tokens、输入拆分、缓存率徽章、输出/时间）；底部"查看全部"跳转

### Bug 修复与优化
- **套餐表头** — 全部使用渐变主题色（不再仅 popular 有颜色）
- **背景统一** — 页面级渐变替代各区块 `bg-muted/30` 交替
- **CTA 区域** — 去除背景框，融入页面背景
- **文档侧边栏** — 从纯色背景改为玻璃卡片
- **统计卡片** — 2×3 网格布局（替代 1×6 拥挤排列）
...
### 网站美化与UX
- **错误边界** — 为所有 Dashboard 子路由添加 error.tsx 错误回退组件
- **加载状态** — 为账单、密钥、API 测试、设置页面添加 loading.tsx 骨架屏
- **共享组件** — 提取 DeltaBadge 为独立组件；ConfirmDialog 加载态添加旋转图标
- **可访问性** — 侧边栏活跃链接添加 `aria-current="page"`
- **渠道管理** — 删除/批量确认改用统一 ConfirmDialog 组件
- **个人资料修复** — 修复 handleSaveProfile 缺少 `await` 导致无法捕获保存错误的问题
- **密码可见性** — 登录和注册表单密码框添加显示/隐藏密码按钮
- **退出重定向** — Auth Context 退出登录后自动跳转到首页
- **日期验证** — 调用日志页面添加客户端日期范围验证（开始日期不能晚于结束日期）
- **模型市场** — 自定义排序下拉替换原生 `<select>`；新增列表/紧凑视图切换
- **API 测试场** — AI 响应支持 Markdown 渲染，替代纯文本 `<pre>` 显示
- **默认主题** — 默认主题优先使用 `prefers-color-scheme` 而非硬编码暗色
- **充值按钮** — 更新禁用充值按钮的提示文案

### 账单与分析
- **账单历史** — 新增描述搜索和类型过滤（充值/扣费/退款/赠送）
- **支出预测** — 新增"预计月消费"卡片（含进度条）
- **月度趋势** — 新增"本月已消费"卡片（含与上月对比 DeltaBadge）
- **模型成本分布** — 调用日志页面新增 ECharts 饼图展示模型费用占比
- **CSV 导出** — 从 window.open 改为 fetch+blob 下载，添加成功/失败 Toast 反馈
- **监控排序** — 管理员监控页面的渠道表支持列排序和文本搜索

### 新功能
- **EmptyState 组件** — 全站统一的空状态展示组件，替换各行其是的空状态
- **服务端路由保护** — 添加 Next.js 16 proxy.ts，对 Dashboard/Profile 路由进行服务端保护
- **错误重试限制** — 错误页面重试按钮最多点击 3 次，超限后禁用

### 第二阶段增强
- **API 测试场** — 对话导出为 Markdown；模型/Key 列表添加刷新按钮
- **侧边栏徽章** — API Keys 菜单项显示活跃 Key 数量
- **预算通知修复** — 修复 StrictMode 下预算超限通知重复触发的问题
- **404 页面** — 动态内容加载，失败时降级到硬编码数据
- **资源中心** — 添加搜索过滤和实时数量统计
- **文档** — 添加"在 GitHub 上编辑此页"链接和"最后更新"日期
- **首页** — 集成热门技能/Prompt 展示和用户评价；新增定价区域（3 档套餐）；Hero 添加滚动指示器
- **Proxy 修复** — 修复 cookie 名称不匹配（token → oortapi_token）

### Bug 修复与优化
- **模型饼图** — 改用全周期 API 聚合（非当前页）；暖/冷色系区分；滚动图例；credits 与金额正确分离
- **筛选栏** — 模型筛选改为下拉框（同步模型市场）；justify-between 布局填满整行
- **SQL 修复** — 修复 created_at 缺少 u. 前缀导致的 ambiguous column 错误
- **默认日期** — 调用日志默认最近 30 天；清除筛选重置为 30 天
- **60 天上限** — 趋势图 API 限制最多 60 天；超限时显示警告条
- **文档目录** — 新增文档页面右侧目录侧边栏（xl 屏幕以上），滚动跟踪当前章节

### 模型能力标签
- **能力标签化** — 模型新增持久化能力标签（推理/视觉/函数调用/长文本），存储于数据库，启动时自动为已知模型注入
- **市场多维筛选** — 新增固定能力过滤器按钮（推理/视觉/函数调用/长文本），不同颜色区分；卡片标签改用 Badge 组件
- **管理员编辑** — 模型编辑模式新增能力复选框，支持勾选/取消标签
- **公共 API 扩展** — `/api/v1/models` 返回 `tags` 字段；后端 POST/PATCH 路由支持 tags 读写
- **测试场解耦** — API 测试场从真实模型数据读取能力，不再依赖 localStorage；思考模式跟随模型能力自动开关；移除手动编辑模态框
- **面包屑移除** — 移除仪表盘布局中冗余的面包屑导航，侧边栏已提供完整导航上下文

### 流式延迟监控与分析
- **TTFT/ITL 追踪** — 流式路由捕获首 Token 到达时间和吐字间隔，持久化到 `usage_logs`
- **延迟监控图** — 仪表盘新增 ECharts 双轴折线图（TTFT 蓝色 / ITL 绿色）
- **缓存 ROI 看板** — 新增环形图展示缓存命中率、节省 Tokens 和费用
- **Stats API 扩展** — `/api/dashboard/stats` 返回 `cache_savings` 缓存节省数据
- **Analytics API 扩展** — `/api/dashboard/analytics` 返回 `latency_trend` 延迟趋势

### 开发者体验
- **文档 Key 注入** — 已登录用户文档页代码块自动填入有效 API Key，替换 `sk-oort-your-key` 占位符
- **Token 计算器** — 模型市场新增内联计算器：选模型 → 填 Tokens → 展示单次/每日/每月/每年费用及套餐 Credits 消耗
- **市场布局优化** — 模型网格改为 4 列；玻璃卡片质感提升（更丰富的阴影和光晕）
...
### 用量统计与分析
- **周期性对比** — 统计 API 返回昨日/上月数据；仪表盘卡片显示涨跌 delta 徽章
- **缓存命中率** — 新增周期缓存命中率指标 + 界面卡片（命中/未命中统计）
- **密钥分析增强** — 支持自定义时间范围；per-model 调用/费用明细
- **模型延迟对比** — 新增模型平均延迟横向柱状图
- **模型错误率** — 新增错误率卡片（绿/黄/红阈值）
- **消费趋势图** — 新增双轴柱+线组合图（每日费用 + 调用量叠加）
- **模型消费** — 堆叠柱状图 tooltip 显示 input/output 拆分

### 计费
- **账单搜索** — 支持按类型/日期范围过滤
- **CSV 导出** — 账单记录导出（上限 10,000 行）
- **月度趋势** — 账单 API 返回 12 个月费用趋势
- **Webhook 事件** — 取消订阅/余额不足(<$1)/续费失败 自动触发通知

### 用户体验
- **调用日志** — 首列（渠道）sticky 固定；延迟颜色编码
- **登录页** — 邮箱输入框自动聚焦
- **页面标题** — 仪表盘子页面动态设置 document.title
- **时区选择** — 从 6 个扩展到 15 个时区（含 UTC 偏移标注）
- **导航栏** — 用户下拉菜单显示当前余额
- **API Keys** — 批量选择/删除（全选 + 计数反馈）
- **头像上传** — 支持拖拽上传图片
- **密钥分析面板** — 新增 per-model 细分列表
...
### 美化
- **Dashboard 布局** — 外层 `<div>` 替换为语义化 `<main>` 标签
- **Dashboard** — `lastUpdated` 时间戳增加日期显示
- **Playground** — AI 回复消息移除 `font-mono`；每条消息添加复制按钮
- **Playground** — 参数面板展开/收起添加 slide-down 动画
- **频道健康概览** — `text-[10px]` 提升为 `text-xs`
- **账单卡片** — 添加"查看套餐"/"订阅"/"联系我们"可操作按钮
- **系统监控** — 自动刷新添加绿色脉冲指示点
- **审计日志** — 添加操作类型筛选下拉框；详情列添加 title tooltip

### 功能
- **频道健康** — 添加加载骨架屏和错误重试按钮
- **Models API** — 删除不存在的模型费率返回 404
- **备份路由** — `require('fs')` 替换为 ESM import
- **调用日志** — 筛选无结果时显示"无匹配记录"提示；跳转页码 clamp 到有效范围
- **调用日志** — 加载态骨架屏改为表格结构（5行）而非单条脉冲条
- **Playground** — max_tokens 默认 4096，上限 131072；localStorage 限制 2MB
- **审计日志** — 分页统一为 total+page 模式；日期范围校验提示
- **套餐管理** — 价格/额度输入禁止负数；删除改用 ConfirmDialog
- **Webhook** — Secret 截断显示添加复制完整值按钮
- **设置页** — 预算 toast 仅在变化时触发（非每次挂载）；预算可清空

### 用户体验
- **Webhook 测试** — 修复 catch 也显示"成功"的错误
- **Webhook 删除** — 原始 Dialog 替换为共享 ConfirmDialog
- **注册页** — 移除"更多登录方式即将上线"未完成提示
- **设置页** — 去除重复的"API Endpoint"标题
- **频道** — 健康数据加载失败显示错误提示+重试按钮
- **个人资料** — 密码输入框添加显示/隐藏切换按钮
- **计费引擎** — 自动续订 CNY 价格正确换算为 USD；fallback 日志记录正确模型名

### 修复
- **路由 404** — 清除 stale 的 `.next` 构建缓存，修复所有 `/api/` 和 `/login` 路由 404
...
### 美化与使用体验优化
- **动画 class 标准化** — 全站 7 处内联任意动画统一为可复用 utility class
- **Token Plan 页面加强** — Hero 区渐变背景 + 装饰光晕 + 错落入场动画
- **套餐对比表格** — Plan 色彩梯度表头 + 星标标记 + 行 hover 高亮
- **FAQ 交互优化** — 升级为手风琴式展开/收起，带旋转指示动画
- **订阅卡片** — 硬编码色值替换为 CSS 变量（主题感知），增加 hover 缩放/阴影
- **新手引导卡片** — 升级为 `glass-card` 样式，匹配 Dashboard 主题
- **StatusBadge** — 暗色模式对比度修复，增加 `transition-colors` 动画
- **复制按钮** — 增加 `group-focus-within` 触摸设备可见性

### UX 改进
- **API Key 删除** — 原生 Dialog 替换为共享 ConfirmDialog 组件
- **取消订阅** — 原生 Dialog 替换为共享 ConfirmDialog
- **设置页确认弹窗** — 原生 `window.confirm()` 替换为自定义 ConfirmDialog
- **设置页表单验证** — 汇率/预算输入增加 min/max 约束
- **设置页错误反馈** — 静默 `.catch()` 替换为 toast 错误提示
- **预算进度条稳定性** — 清空输入后进度条不消失（回退到已保存值）
- **API Key 创建** — 增加名称必填验证 + 过期日期 min 属性（禁止过去日期）
- **API Key 状态标签** — 统一使用 StatusBadge 组件
- **充值按钮 tooltip** — 禁用按钮悬停提示「即将上线」
- **兑换码提示** — 增加自动大写转换的小字提示
- **Playground** — 聊天记录/模型选择/密钥选择/参数/系统提示持久化到 localStorage
...
- **Usage API** — 聚合和趋势查询增加 Token 明细字段
- **Analytics API** — 查询更新为三分类模型

### 展示更新
- **调用日志** — 列顺序：输入(未命中缓存) → 输入(命中缓存) → 输出 → 总计
- **调用日志卡片** — Token 卡片显示三分类明细
- **概览 StatsCards** — Token 明细更新为三分类
- **资源使用图表** — Tooltip 和汇总使用三分类
- **模型市场** — 价格卡显示：输入(未命中缓存)、输入(命中缓存)、输出
- **模型详情** — 同步更新价格标签
- **全站** — 统一标签：输入(未命中缓存)、输入(命中缓存)、输出
...
### 数据一致性
- **成本精度统一** — 全站统一使用 `formatPrice` 上下文，统一小数位
- **货币系统统一** — 模型市场改用共享 `useCurrency()` 上下文
- **Token 单位** — 全站统一 `/1M tokens`
- **日期时区** — 修复兑换码和 API Key 过期日期的 +Z 后缀缺失
- **搜索标签** — 统一为 "Input"/"Output"
- **Profile 统计** — 改用服务端聚合数据，不再仅取最近 10 条

### 管理面板 UX
- **删除确认** — 对话框显示被删除对象的实际名称/码值
- **updated_at 展示** — Webhook 和套餐显示最后修改时间
- **设置确认** — 汇率变更前弹出确认对话框
- **对话框焦点** — 所有创建/编辑弹窗自动聚焦首个输入框
- **URL 提示** — 截断的 Webhook URL 悬停显示完整地址
- **调用次数格式化** — API Key 列表使用 locale 数字格式
- **渠道测试结果** — 每个渠道独立维护测试状态
- **总数角标** — API Keys 和兑换码列表显示总数

### 共享组件
- **StatCard** — 可复用统计卡片组件（8 种颜色）
- **StatusBadge** — 可复用状态徽标（成功/错误/警告/信息/静默）
- **ConfirmDialog** — 可复用确认对话框（支持危险操作变体）
- **LoadingSkeleton** — 可复用加载骨架屏
...
### 新功能
- **密码重置** — 新增忘记密码流程（`/forgot-password` + `/reset-password`）
- **记住我** — 登录页添加「记住我」复选框
- **API Key 过期** — 支持设置 Key 过期日期，显示倒计时/已过期状态
- **创建套餐** — 管理员可在后台直接创建订阅套餐
- **套餐对比表** — Token Plan 页新增功能对比表格

### 体验改进
- **Playground** — 多轮对话历史、可配置参数（temperature/max_tokens/top_p）、系统提示词、响应复制按钮
- **调用日志** — 表头点击排序（Token、费用、延迟、时间）
- **模型详情** — 未登录用户显示「注册使用」CTA，已登录用户显示「在线测试」
- **模型市场** — 添加「加载更多」分页（每批 20 个）
- **API Key 搜索** — 按名称/Key 值搜索，删除对话框显示 Key 名称
- **渠道密钥显隐** — 密钥输入框添加眼睛切换按钮
- **渠道费用统计** — 渠道健康数据增加 24h 费用显示
- **Webhook 状态** — 显示最后触发时间和状态码
- **用户 CSV 导出** — 管理员可导出用户列表
- **兑换码搜索** — 按码值搜索，单行复制按钮
- **SDK 文档** — 动态 Base URL，不再硬编码 localhost
- **文档搜索** — 文档首页添加搜索输入框
- **快捷键** — Ctrl+K 快捷键现在可用

### 控制台增强
- **概览页** — 错误状态+重试、手动刷新、上次更新时间
- **系统监控** — 30 秒自动刷新、手动刷新、ECharts 24h 趋势图
- **账单页** — 余额刷新按钮
- **设置页** — 预算超限时 Toast 通知

### 其他
- **GitHub 链接** — 页脚 GitHub 链接指向实际仓库
- **注册页** — 显示「更多登录方式即将上线」提示
...
### 重大 — 文档全面重构
- **多页文档体系** — 将单页 920 行文档拆分为 10 个子页面，带侧边栏导航：快速开始、认证方式、API 端点、SDK 集成、流式响应、定价说明、错误码、速率限制、部署指南、集成指南
- **文档侧边栏** — 分组折叠导航，客户端搜索过滤，移动端适配
- **文档首页** — 分类卡片导航布局，替代原有单页堆叠
- **共享组件** — 提取 CodeBlock、BaseUrlDisplay、EndpointRow、DocsSidebar 为可复用组件
- **i18n 扩展** — apiDocs 命名空间从 ~20 键扩展到 ~90 键，完整双语支持

### 新页面
- **版本日志** — `/changelog` 从 GitHub 加载 CHANGELOG 渲染版本历史
- **常见问题** — `/faq` 10 条常见问题（展开式，双语）
- **速率限制** — `/docs/rate-limits` 含重试策略代码示例（Python/Node.js）
- **集成指南** — `/docs/integrations` 逐应用配置指南

### 改进
- **页脚** — 新增 FAQ 链接、系统运行状态指示器（轮询 `/api/health`）
- **导航栏** — 移除 changelog/FAQ（仅保留在页脚）
- **构建** — 项目现在生成 181 个静态页面（原 169）
...
### 新功能
- **用量趋势图** — 调用日志上方新增 ECharts 柱状图，支持切换「费用/Token/调用次数」维度，与筛选联动
- **用量汇总统计** — 顶部汇总卡片改为全量聚合（不再只统计当前页），新增每日趋势数据
- **按 API Key 筛选** — 调用日志支持按 API Key 下拉筛选
- **Key 速率限制** — API Key 卡片显示并支持行内编辑 RPM 速率限制
- **Key 用量分析** — 每个 Key 可展开查看 7 天统计：调用次数、费用、Tokens、平均延迟、错误率
- **预算管理** — 设置页新增月度预算上限，带进度条和阈值提醒
- **系统监控** — 管理员新增 `/dashboard/admin/monitor` 页面：QPS、错误率、P50/P95 延迟、按供应商健康状态（自动刷新）
- **API 测试场** — 新增 `/dashboard/playground` 页面：选择模型 + API Key，发送消息，流式显示响应
- **渠道搜索筛选** — 渠道列表支持名称搜索、按状态（在线/离线/启用/禁用）和供应商类型筛选
- **渠道批量操作** — 多选 checkbox，支持批量启用/禁用/删除
- **渠道路由矩阵** — 可切换视图，展示模型到渠道的映射关系及优先级/权重
- **乘数生效价格** — 表格展示生效输入/输出价（基础费率 × 倍率），支持批量编辑
- **审计日志导出** — 新增 CSV 导出按钮，搜索扩展到 action + details 全文搜索
- **用户活动时间线** — 管理员用户详情页新增最近 20 条调用记录

### 改进
- **模型详情汇率** — 使用系统汇率设置，不再硬编码 7.3
- **搜索结果跳转** — 搜索结果链接改为模型详情页 `/models/{id}`，而非列表页
- **首页服务商** — ModelWall 从 API 动态获取实际供应商列表，不再硬编码
- **充值按钮提示** — 充值按钮下方显示可见提示文字，引导使用兑换码
- **Toast 位置** — 移至 bottom-20，避免与回到顶部按钮重叠
...
### 新功能
- **获取上游模型** — 渠道管理新增"获取模型"按钮，调用上游 `/v1/models` 接口，弹框展示可用模型列表（带复选框），勾选后自动填入。新建和编辑渠道均可使用。
...
### 新功能
- **Docker 一键部署** — 新增 `Dockerfile`（多阶段构建）和 `docker-compose.yml`，一条命令部署，SQLite 数据持久化
- **首注册自动管理员** — 第一个注册的用户自动成为管理员，管理员可在用户管理中提升其他用户
- **删除账户** — 个人中心支持删除账户，需密码验证 + 输入 "DELETE" 确认
- **修改密码移至个人中心** — 修改密码从控制台设置移至个人中心设置 tab，更符合用户直觉

### 修复
- **调用日志 SQL 错误** — 修复 COUNT 查询别名不匹配（`u.user_id` → `user_id`）导致日志为空
- **调用日志筛选** — 添加手动筛选按钮，筛选仅在点击时生效，进入页面默认显示全部记录
- **日期范围筛选** — 修复"截止日期"不包含当天记录的问题（`created_at < date(?, '+1 day')`）
- **Docker schema.sql** — 修复 `schema.sql` 未复制到 Docker 镜像导致注册失败

### 变更
- **控制台设置精简** — 移除账户设置（用户名/简介）和修改密码（已移至个人中心）；控制台设置仅保留 API 端点 + 系统设置（管理员）
...
### 关键修复
- **渠道延迟统计 BUG** — 修复 SQL 查询引用错误列 `api_key_id` → `channel_id`
- **登录暴力破解防护** — 添加 IP 级别速率限制（60 秒内 5 次尝试，超限后锁定 15 分钟）
- **API Key 哈希存储** — API Key 现在以 SHA-256 哈希存储，旧 Key 首次使用时自动迁移

### 新功能 — 开发者体验
- **用量日志筛选** — 调用日志页添加模型搜索、状态筛选、日期范围选择器
- **CSV 导出** — 用量日志支持 `?format=csv` 导出 CSV，UI 添加下载按钮
- **模型详情页** — 每个模型现在有独立页面展示定价、7 天统计、可用渠道（`/models/[model]`）
- **模型标签** — 模型支持分类标签（如 `vision`、`reasoning`），模型页可按标签筛选
- **OpenAPI Spec 统一** — `/api/v1/openapi` 和 `/api/docs/openapi.json` 现在使用同一份完整规范

### 新功能 — 平台能力
- **审计日志 UI** — `/dashboard/admin/audit` 完整审计日志页面，支持按操作类型、目标类型、日期筛选
- **Webhook 管理** — `/dashboard/admin/webhooks` CRUD 管理，支持事件配置和测试发送
- **渠道健康监控** — 渠道页新增 24h 成功率、平均延迟、状态总览卡片
- **渠道健康历史** — 新增 `channel_health_log` 表，记录状态变化，支持 7 天趋势展示
- **用户活跃度统计** — `/dashboard/users/[id]` 用户详情页展示总调用、总 Token、总消费、常用模型、7 天趋势
- **套餐分析面板** — 管理员套餐页新增各套餐订阅人数、月收入、Credits 使用率
- **配置导出/导入** — 设置页支持导出完整配置（设置 + 渠道 + 模型 + 套餐）为 JSON 并导入
- **兑换码批量操作** — 支持复选框多选兑换码，批量启用/禁用/删除
- **用户批量操作** — 支持多选用户，批量启用/禁用和批量发放余额
- **API 调用实时日志** — 调用日志页每 30 秒自动刷新，显示实时指示器、暂停按钮、新行高亮
- **计划升级/降级** — 用户可更换套餐，剩余天数额度按比例折算到新套餐
- **订阅到期提醒** — 仪表盘在订阅到期前 3 天（自动续费关闭时）显示警告横幅

### 数据库迁移
- `model_rates.tags` — 新增 TEXT 列用于模型分类标签
- `usage_logs.is_stream` — 新增 INTEGER 列记录流式请求
- `usage_logs.request_size_bytes` — 新增 INTEGER 列记录请求体大小
- `channel_health_log` — 新表，记录渠道健康历史
...
### 关键修复
- **引导链接** — 修复错误链接 `/dashboard/api-keys` → `/dashboard/keys`
- **API Key 创建 toast** — 修复错误消息（显示"已复制"而非"API Key 创建成功"）
- **渠道 PATCH 存在性检查** — 更新不存在的渠道现在返回 404 而非静默 200
- **渠道 API Key 掩码** — 渠道列表现在显示 `••••••••abcd` 而非泄露前 10 位

### 用户体验改进
- **用量图表货币** — tooltip 现在使用货币上下文而非硬编码 `$`
- **兑换 toast** — 兑换成功消息现在使用动态货币符号
- **统计卡片标签** — "Total Spent" 更名为 "Monthly Cost" 以匹配实际数据
- **API Key 最后使用** — 现在显示相对时间（"3 小时前"）而非原始 ISO 字符串
- **用量展开行** — 添加关闭 (X) 按钮用于折叠详情行
- **倍率删除确认** — 删除现在需要确认对话框
- **用户页确认** — 取消订阅、赠送、添加额度现在需要确认
- **用户搜索防抖** — 搜索输入现在 300ms 防抖而非每次按键都发请求
- **兑换表单验证** — 空/无效金额现在显示错误而非提交
- **账单历史分页** — 现在分页（每页 20 条）而非加载全部记录
- **账单 API 分页** — `/api/dashboard/billing` 现在支持 `limit` 和 `offset` 参数

### 国际化
- **Hero 统计** — "30+ AI Models"、"< 200ms Latency"、"99.9% Uptime" 现在双语
- **文档 CopyButton** — 复制按钮 title 从硬编码"复制"改为双语
- **仪表盘快速开始** — 代码块现在有复制按钮（hover 显示）

### UI/交互
- **侧边栏活跃状态** — 修复过宽匹配（如 `/dashboard/settings` 不再为 `/dashboard/setting` 高亮）
- **Footer 定价链接** — 修复"Pricing"链接指向 `/models` 而非 `/token-plan`
- **导航栏 Dashboard 链接** — 未登录用户不再看到 Dashboard 链接
- **移动端 Sheet 切换** — 移动端导航 Sheet 现在包含语言和主题切换
- **仪表盘面包屑** — 根据 URL 路径自动生成面包屑导航
- **用量图表骨架屏** — 加载状态现在显示动画卡片骨架而非空白
- **侧边栏移动端折叠** — 移动端导航菜单现在可折叠，带箭头切换按钮
- **原生 select 样式** — 所有 `<select>` 元素（设置、倍率）现在使用统一的表单输入样式

### 国际化（第二批）
- **文档页完整 i18n** — 所有章节标题、端点描述、错误码、定价层级、认证描述现在双语（通过 DOCS_LABELS）

### 后端/安全
- **邮箱验证** — 注册现在服务端验证邮箱格式
- **渠道 PATCH** — 无字段时返回 400，渠道不存在时返回 404
- **用户名长度** — 个人资料用户名限制统一为 2-32 字符（原为 2-50）
...
#### 新用户引导
- **仪表盘 onboarding** — 新用户（无 API Key + 无订阅）现在看到 3 步引导卡片：创建 Key → 选择套餐 → 开始调用

#### 移动端适配
- **用量表格响应式** — 14 列用量表格在移动端自动隐藏次要列（缓存 token、倍率、延迟、备注），仅显示核心信息（模型、总量、费用、状态、时间）

#### 错误反馈
- **管理员页面错误提示** — multiplier、channel-card、admin/plans 页面的所有 CRUD 操作现在显示成功/失败 toast 通知（之前静默吞掉错误）

#### 文档改进
- **代码块复制按钮** — 文档页面所有代码块右上角添加复制按钮（hover 显示）

#### 表单与交互
- **注册密码提示** — 密码输入框下方显示"至少 8 个字符"提示（输入前可见）
- **充值按钮** — 充值按钮改为 disabled + tooltip 提示"即将上线，请使用兑换码"（之前点击弹 toast）

#### 国际化
- **Hero 徽章** — 首页"Unified AI API Gateway"徽章现在支持中英文切换
- **Auth 错误边界** — 错误回退界面同时显示中英文提示

#### 按钮一致性
- **删除确认按钮** — 所有删除对话框统一使用 `bg-red-600` 样式（之前部分用 `variant="destructive"`，样式不一致）
