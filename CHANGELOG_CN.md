...
### 功能改进
- **详情页入口** — Prompt 模板卡片新增「详情」按钮跳转至 `/prompts/{id}` 详情页，客户端技能卡片跳转至 `/skills/{id}`
- **分页展示** — 「全部」Tab 每类展示 3 张卡片（MCP/Prompt/技能各 3），带分区标题和「查看更多」跳转至对应 Tab；具体 Tab 默认 9 张，超出可展开
- **Tab 重排序** — 调整为「全部 → 云端 MCP → Prompt 模板 → 客户端技能」，技能移至末位
- **i18n** — 新增 `viewDetail`、`viewMore` 词条
