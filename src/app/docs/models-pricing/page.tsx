    ...
    <div className="space-y-10">
      ...
      <div>
        <h1 className="text-2xl font-bold tracking-tight mb-2">
          {lang === "zh" ? "模型与定价" : "Models & Pricing"}
        </h1>
        <p className="text-muted-foreground">
          {lang === "zh"
            ? "OortAPI 聚合多家 AI 服务商，提供统一的 OpenAI / Anthropic 兼容接口。所有模型按实际 token 用量计费。"
            : "OortAPI aggregates multiple AI providers behind a unified OpenAI / Anthropic compatible interface. All models are billed by actual token usage."}
        </p>
      </div>
      ...
      <section className="space-y-4">
              ...
              {POPULAR_MODELS.map((model) => (
                <tr key={model.name} className="border-b border-border/20 last:border-b-0 hover:bg-muted/5 transition-colors">
                  <td className="px-4 py-3">
                    <span className="text-xs px-2 py-0.5 rounded-full bg-primary/10 text-primary font-medium">{model.provider}</span>
                  </td>
        ...
        <p className="text-xs text-muted-foreground">
          {lang === "zh"
            ? "以上为示例价格，实际价格以模型市场页面为准。"
            : "Prices shown are examples. Check the model marketplace for current rates."}
        </p>
      </section>
      ...
      <section className="space-y-4">
        ...
        <p className="text-sm text-muted-foreground">
          {lang === "zh"
            ? "调用此接口获取所有可用模型及其定价信息："
            : "Call this endpoint to get all available models and their pricing:"}
        </p>
        <CodeBlock examples={[
      ...
      <div className="flex flex-wrap gap-3">
        ...
        >
          {lang === "zh" ? "浏览完整模型市场" : "Browse Full Model Marketplace"}
        ...
        >
          {lang === "zh" ? "查看详细定价规则" : "View Detailed Pricing Rules"}
