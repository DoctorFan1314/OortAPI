...
/* ------------------------------------------------------------------ */
/*  Code examples (static, outside component to avoid re-alloc)       */
/* ------------------------------------------------------------------ */
...
const sections: FeatureSection[] = [
  {
    ...
    titleZh: "流式输出",
    descEn:
      "OortAPI supports real-time streaming for both OpenAI and Anthropic protocols.",
    descZh:
      "OortAPI 同时支持 OpenAI 和 Anthropic 协议的实时流式输出。",
    bulletsEn: [
      "SSE (Server-Sent Events) format",
      "Use stream_options.include_usage for token counts",
      "Both OpenAI and Anthropic streaming protocols supported",
    ],
    bulletsZh: [
      "SSE（Server-Sent Events）格式",
      "通过 stream_options.include_usage 获取 token 用量",
      "同时支持 OpenAI 和 Anthropic 流式协议",
    ],
    code: streamingCode,
  },
  {
    ...
    titleZh: "函数 / 工具调用",
    descEn:
      "Use tools and function calling through the relay — fully compatible with OpenAI and Anthropic tool formats.",
    descZh:
      "通过中继使用工具和函数调用 — 完全兼容 OpenAI 和 Anthropic 工具格式。",
    bulletsEn: [
  ...
  },
  {
    ...
    descEn:
      "Get structured JSON responses using the response_format parameter.",
    descZh:
      "通过 response_format 参数获取结构化 JSON 响应。",
    bulletsEn: [
      "JSON mode for free-form JSON output",
      "Structured outputs with JSON Schema validation",
    ],
    bulletsZh: [
      "JSON 模式：自由格式的 JSON 输出",
      "结构化输出：基于 JSON Schema 校验",
    ],
  ...
  },
  {
    ...
    titleZh: "视觉与多模态",
    descEn:
      "Send images and multimodal content to vision-capable models.",
    descZh:
      "向支持视觉的模型发送图片和多模态内容。",
    bulletsEn: [
      "Supports GPT-4o, Claude 3.5 Sonnet, Gemini, and more",
      "Base64 and URL image inputs",
    ],
    bulletsZh: [
      "支持 GPT-4o、Claude 3.5 Sonnet、Gemini 等模型",
      "支持 Base64 和 URL 图片输入",
    ],
    code: visionCode,
  },
  {
    ...
    titleZh: "提示词缓存",
    descEn:
      "OortAPI supports prompt caching for eligible models, reducing costs by up to 90%.",
    descZh:
      "OortAPI 支持符合条件的模型的提示词缓存，最高可节省 90% 成本。",
    bulletsEn: [
      "Automatic caching for Anthropic models via cache_control",
      "OpenAI caching is automatic — no code changes needed",
      "Significant cost savings on repeated prompts",
    ],
    bulletsZh: [
      "Anthropic 模型通过 cache_control 自动缓存",
      "OpenAI 缓存自动生效，无需修改代码",
      "重复提示词可大幅节省成本",
    ],
  ...
  },
  {
    ...
    titleZh: "模型路由",
    descEn:
      "Intelligent routing across multiple upstream channels for reliability.",
    descZh:
      "智能路由多个上游通道，确保高可用性。",
    bulletsEn: [
      "Weight-based routing across channels",
      "Automatic failover on upstream errors",
      ...
      {sections.map((s) => {
        ...
        return (
          <section
            key={s.id}
            id={s.id}
            className="rounded-xl border border-border/50 overflow-hidden glass-card"
          >
            {/* Card header */}
