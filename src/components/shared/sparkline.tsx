...
export function Sparkline({ data, width = 80, height = 24, color = "#0891b2", className, gradientId }: SparklineProps) {
  const path = useMemo(() => {
    ...
    return `M0,${height} L0,${top} L${points.join(" L")} L${width},${height} Z`;
  }, [data, width, height]);

  if (!path) return null;
  const gid = gradientId || "spark-def";
