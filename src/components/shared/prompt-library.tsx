...
export function PromptLibrary({ onSelect, open, onClose }: PromptLibraryProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4" role="dialog" aria-modal="true" aria-label={p.title} onClick={onClose}>
      <div className="bg-card border border-border/70 rounded-xl p-5 max-w-md w-full mx-auto shadow-2xl max-h-[80vh] flex flex-col animate-page-fade-in" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-sm font-semibold text-foreground flex items-center gap-2"><BookOpen className="h-4 w-4 text-primary" />{p.title}</h2>
          <button onClick={onClose} className="p-1 rounded-md hover:bg-muted transition-colors" aria-label={p.close}><X className="h-4 w-4 text-muted-foreground" /></button>
        </div>

        {/* Template list */}
        <div className="flex-1 overflow-y-auto space-y-1 min-h-0 mb-3">
          {templates.length === 0 ? (
            <p className="text-xs text-muted-foreground text-center py-6">{p.noTemplates}</p>
          ) : (
            templates.map((tpl) => (
        ...
        <div className="space-y-2 pt-3 border-t border-border/30">
          <input value={newName} onChange={(e) => setNewName(e.target.value)} placeholder={p.templateName} className="w-full h-8 px-2.5 rounded-md border border-input bg-background text-xs focus:border-primary focus:outline-none" />
          <textarea value={newContent} onChange={(e) => setNewContent(e.target.value)} placeholder={p.promptContent} rows={3} className="w-full px-2.5 py-1.5 rounded-md border border-input bg-background text-xs font-mono resize-none focus:border-primary focus:outline-none" />
          <button onClick={addTemplate} disabled={!newName.trim() || !newContent.trim()} className="flex items-center gap-1.5 px-3 py-1.5 rounded-md bg-primary text-primary-foreground text-xs font-medium hover:bg-primary/90 transition-colors disabled:opacity-50">
            <Save className="h-3 w-3" />{p.saveTemplate}
          </button>
        </div>
      </div>
    </div>
