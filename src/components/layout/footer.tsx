            ...
            <nav key={section.id} aria-label={section.title}>
              ...
              <ul className="space-y-2.5">
                {section.links.map((link) => (
                  <li key={link.label}>
                    {link.disabled ? (
                      <span className="text-sm text-muted-foreground/50 cursor-not-allowed" aria-disabled="true" tabIndex={-1} onClick={(e) => e.preventDefault()}>{link.label} <span className="text-[10px]">({t.footer.comingSoon})</span></span>
                    ) : (
