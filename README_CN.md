...
| npm | >= 9.0 |
| C++ 编译工具 | Windows: [VS Build Tools](https://visualstudio.microsoft.com/visual-cpp-build-tools/)（勾选 "Desktop development with C++"）<br>macOS: `xcode-select --install`<br>Linux: `build-essential` / `python3` / `make` / `g++` |
...
- **用量趋势图** — ECharts 柱状图，支持费用/Token/调用切换，与筛选联动
- **系统监控** — 管理员仪表盘：QPS、错误率、P50/P95 延迟、按供应商健康状态
- **API 测试场** — 交互式测试：选择模型 + Key，多轮对话，可配置参数，流式响应，MCP 工具循环，Switch 开关工具管理器（模型能力联动）
- **资源中心** — AI 应用市场：17 个真实开源 MCP 节点（来自 modelcontextprotocol/servers）、28+ Prompt 模板、客户端技能，一键激活至测试场
- **多页文档** — 18 个文档页面：CodeBlock 行号/高亮行、安全与隐私页、上下页导航、FAQ 搜索、请求参数表（5 列含默认值）、全部端点响应示例、Python/Node.js/cURL 多语言示例、TPM 速率限制文档、CrossLinks 交叉引用组件、SEO metadata wrapper、全站 i18n（240+ 控制台键集中管理）
- **渠道路由矩阵** — 可视化模型到渠道映射，含优先级/权重
- **预算管理** — 月度消费上限，阈值提醒
- **Key 用量分析** — 每个 API Key 的 7 天统计（调用、费用、Tokens、延迟、错误率）
...
- **用量分析** — 详细调用历史，含 ECharts 趋势图、API Key 筛选、全量汇总统计、Token 分拆（输入、输出、缓存命中、缓存创建），货币感知费用展示，时区正确的时间戳
- **账单** — 余额展示（USD/CNY）、交易历史、兑换码充值
- **渠道管理** — 管理员配置 AI 服务商渠道，支持智能路由、连接测试、模型同步、健康监控（24h 成功率、延迟、调用次数）、搜索筛选、批量操作、路由矩阵
- **用户管理** — 管理员管理用户，角色控制、余额调整、启用/禁用、密码重置、近期活动时间线
- **兑换码** — 管理员批量生成兑换码，用户即时兑换充值
- **套餐管理** — 管理员在 `/dashboard/admin/plans` 管理订阅套餐，支持货币切换和套餐-模型绑定
- **倍率规则** — 管理员配置按模型和按时段的定价倍率，支持生效价格显示和批量编辑
- **系统监控** — 管理员实时平台健康 `/dashboard/admin/monitor`（QPS、错误率、P50/P95 延迟、按供应商统计）
- **API 测试场** — 交互式 API 测试 `/dashboard/playground`，模型选择、流式响应、Markdown 渲染、对话导出、MCP 工具循环（Tool Loop）、Switch 开关工具管理器（模型能力联动）、Tavily 搜索 Key 配置
- **预算管理** — 月度消费上限，进度条、阈值提醒和支出预测
- **审计日志** — 管理员全文搜索、CSV 导出、日期范围筛选
...
- **连接测试** — 验证上游连通性并测量延迟
- **模型同步** — 一键将渠道模型同步到模型市场
- **获取模型** — 从上游 `/v1/models` 获取可用模型列表，弹框勾选后自动填入
...
资源中心枢纽页 `/resources/` 提供三大资源维度的统一入口：
